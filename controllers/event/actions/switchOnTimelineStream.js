...
/**
 * The action open the stream of the event's timeline and update the timeline status of the event
 * in the DDBB.
 *
    ...
    function (err, event) {
      if (err) {
        helperGlobal.addError(req,
          new iWAErrors.Db('Controller: event # Action: switchOnTimelineStream ' +
      ...
      if (!event) {
        helperGlobal.addError(req, new iWAErrors.HttpRequest('Controller: event # Action: ' +
          'switchOnTimelineStream | The event doesn\'t exist. Event id ' + eventId, 404, req), 404);
        sendResponse(req, res, next, post);
        return;
      }

      if (timelineStream.open(event) === false) {
        helperGlobal.addError(req, new iWAErrors.ClientAttack('Controller: event # Action: ' +
          'switchOnTimelineStream | The stream of the event with id: ' + eventId +
          ' was already opened, so the call may come out of the web application\'s frontend', 603,
