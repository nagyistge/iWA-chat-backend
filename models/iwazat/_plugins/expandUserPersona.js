 ...
 */
function ExpandUserPersonaPlugin(schema, options) {
  ...
  if ((!currentSchema.paths[tmpPath + idFielName]) || (!currentSchema.paths[tmpPath + 'persona'])) {
    throw new Error('Invalid path type; the field of the attribute ' + tmpPath
      + ' is not nested type');
  }
  ...
  schema.methods[methodName] = function (userCallback) {
    ...
    var populate = function (cDocField, callback) {
      ...
      collector.on('error', function (err, pos) {
        ...
        if (populatedCounter === populatedUsers.length) {
          callback(errors, populatedUsers);
        }
...
//        Return javascript plain object rather than mongoose documents instances
        model.find( {
            ...
            personas: { $elemMatch: { _id: elements[it].persona}}
          },
...
module.exports = ExpandUserPersonaPlugin;